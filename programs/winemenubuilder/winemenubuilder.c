                                ...
                                const WCHAR *workdir, const WCHAR *icon, const WCHAR *wmclass)
{
    ...
    fprintf(file, "spark-wine10-wow %s", escape(path));
                                                ...
                                                const WCHAR *openWithIcon)
{
    ...
    if (desktop)
    {
        ...
        {
            char *path = wine_get_unix_file_name( prefix );
            fprintf(desktop, "Exec=env WINEPREFIX=\"%s\" spark-wine10-wow start /ProgIDOpen %s %%f\n", path, escape(progId));
            heap_free( path );
        }
        else
            fprintf(desktop, "Exec=spark-wine10-wow start /ProgIDOpen %s %%f\n", escape(progId));
